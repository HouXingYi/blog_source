...
4. 最大堆的建立
    ...
    方式1：通过插入操作，将N个元素一个个相继插入到一个初始为空的堆中去，其时间代价最大为O（NlogN）。
    方式2：在线性时间复杂度下建立最大堆
        1）将N个元素按输入顺序存入，先满足完全二叉树的结构特性。
        2）调整各结点位置，以满足最大堆的有序特性。

    思路：类似删除中调整的思路，从上到下对比交换。从最后一个结点的父结点开始。
...
包含：
    图：通常G表示。
    一组顶点：通常用V表示顶点集合。|V|表示图中顶点的个数。
    一组边：通常用E表示边的集合。|E|表示图中边的条数。
        （V，W）无向边
        <V，W>有向边
...
深度优先搜索（Depth First Search, DFS）
    类似于树的先序遍历，利用堆栈。
    首先访问某一起始顶点v，接着访问与v相邻的任一未访问的结点，依次递归。当不能再继续向下访问时，依次退回到最近被访问的顶点，若它还有邻接点未被访问过，则从该点重复以上过程。
    算法时间复杂度为O（|V| + |E|）
    
广度优先搜索（Breadth First Search, BFS）
    类似于树的层序遍历，利用队列。
    首先访问结点v，接着访问结点v各个未访问过的邻接结点，依次递归。
    Dijkstra单源最短路径算法和Prim最小生成树算法都应用了类似的思想。
    算法时间复杂度为O（|V|^2）
...
单源最短路径问题：从某固定源出发，求其到所有其他顶点的最短路径问题。

多源最短路径问题：求任意两顶点间的最短路径。

### 无权图单源最短路算法：

按照递增的顺序找出到各个顶点的最短路：类似BFS

dist[W] = S到W的最短距离
dist[S] = 0
path[W] = S到W的路上经过的某顶点 // 可以记录路径，回溯找回路径

与BFS的区别：通过dist表示与source的距离，通过path来记录回溯的路径。
...
无权图可以认为是特殊的有权图，每个边的权重为1

Dijkstra算法

    * S = {源点s + 已经确定了最短路径的顶点Vi}
    * 对任一未收录的顶点v，定义dist[v]为s到v的最短路径长度，但该路径仅经过s（表示已确定最短路径）中的顶点。
    * 若路径是按照递增的顺序生成的，则
        真正的最短路径必须只经过s中的顶点：因为是递增的
        每次从未收录的顶点中选一个dist最小的收录（贪心）
        增加一个v进入s，可能影响另外一个w的dist值

思路：从当前情况下，找一个未被处理过的dist最小的出来（称之为V），V打上已处理的标签，接着处理这个结点V的邻接点（for循环）并更新邻接点的最小距离和对应的路径，接着重复以上。
...
void Dijkstra (Vertex s) {
	while (1) {
		...
		collected[V] = true; // 收录
		for (V的每个邻接点W) {
			if (collected[W] == false) { // 必须是未被收录的顶点（即有被for循环过邻接点的顶点）
				if (dist[V] + E<V, W> < dist[W]) { // 寻找V的邻接点W中，V到原点的距离+V到W的距离的和最小的那个。
                    // 更新W的距离与路径
					dist[W] = dist[V] + E<V, W>;
					path[W] = V;
				}
...
D[i][j]即为vi到vj的最短路径长度

随着算法运行，D[i][j]逐步被更新为最终的最短路径
...
意义：包含图中的所有顶点，并且只含尽可能少的边，并且生成树的权值之和最小。
...
每个顶点构成一棵独立的树，直接依次找最小的边，只要不构成回路即加入树种，将各个小树合成大树。
...
AOV网（activity on vertax）：顶点表示活动
...
在插入排序的基础上克服每次只交换相邻两个元素的问题。

希尔排序算法的整体时间复杂度和增量序列的选取有关，目前并没有统一的最优增量序列。

算法思路：将序列用不同的间隔分开，按间隔排序，间隔逐渐减少，最终间隔为减少为1。
...
void Heap_Sort (ElementType A[], int N) {
    ...
    for (i = N - 1; i > 0; i--) {
        // 将堆顶元素（最大）与当前堆的最后一个元素A[i]（最小）的元素进行交换
        Swap(&A[0], &A[i]);
        // i以前的重新调整为新的最大堆
        Adjust[A, 0, i];
    }
...
核心：有序子列的归并 merge，即为两个有序序列的合并问题。
    只传入一个原始序列，直接将序列拆开，只传入左指针，右指针，右终点。左终点为右指针的前一位
...
思路：选Pivot，比Pivot大的元素从右向左放置，比Pivot小的元素从左向右放置，接着递归以上步骤。
...
1. 将基准与最后一个交换
2. 两个指针分别指向第一个和倒数第二个，分别为left和right。
3. left向右扫描，right向左扫描。
4. left遇到比基准大的停止，right遇到比基准小的停止，两边都找到后，相互交换（保证基准左小右大）。
5. 重复第3步与第4步，直到left>right。将基准与left交换，放到中间。

以上步骤将基准分为左边小于基准，右边大于基准，接下来依次递归左边和右边。

## 表排序

数据太大，不好移动与交换。只移动指向数据的指针，不移动数据。

不物理排序的情况
    新建一个表数组，对应其数据，只对表数组中的数据进行排序，而无需真正的交换数据。
...
N个学生，成绩是0到100之间（于是有M = 101个不同的成绩值）。如何在线性时间内将学生按成绩排序。

于是有101个桶，每个学生的成绩装在桶，一个桶可能有多个成绩。比如有2个人10分，3个人11分，etc。
...
分为线性探测（偏移为线性），平方探测（偏移为平方），双散列（）。

2. 链地址法：同一个位置的冲突对象组织在一起

### 开放地址法

1. 线性探测法

循环试探下一个位置（一直+1，直到有空位为止）
...
int Insert(HashTable H, int num) {
	int pos, i;
	pos = num % H->size;
	for (i = 0; i <= H->size/2 && H->list[pos]; i++) //增量 <= size/2
